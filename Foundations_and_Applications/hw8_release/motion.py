...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        G = np.array([[np.sum(np.square(Ix[y1-w:y1+w+1, x1-w:x1+w+1])),
                       np.sum(Ix[y1-w:y1+w+1, x1-w:x1+w+1]*Iy[y1-w:y1+w+1, x1-w:x1+w+1])],
                      [np.sum(Ix[y1-w:y1+w+1, x1-w:x1+w+1]*Iy[y1-w:y1+w+1, x1-w:x1+w+1]),
                       np.sum(np.square(Iy[y1-w:y1+w+1, x1-w:x1+w+1]))]
                     ])
        assert G.shape == (2,2)
        ### END YOUR CODE
        ...
        for k in range(num_iters):
            ...
            ### YOUR CODE HERE
            # 边界检查
            if img1[y1-w:y1+w+1, x1-w:x1+w+1].shape != img2[y2-w:y2+w+1, x2-w:x2+w+1].shape:
                continue
            
            bk = np.array([[np.sum((img1[y1-w:y1+w+1, x1-w:x1+w+1]-img2[y2-w:y2+w+1, x2-w:x2+w+1]) * 
                                    Ix[y1-w:y1+w+1, x1-w:x1+w+1])],
                           [np.sum((img1[y1-w:y1+w+1, x1-w:x1+w+1]-img2[y2-w:y2+w+1, x2-w:x2+w+1]) *
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        ### YOUR CODE HERE
        l_keypoints = keypoints/pow(scale, L)
        d = iterative_lucas_kanade(pyramid1[L], pyramid2[L], l_keypoints, window_size,
                                     num_iters, g)
        if level == 0:
            break
        g = scale*(g+d)
...
def IoU(bbox1, bbox2):
    ...
    InterX1 = max(x1, x2)
    InterY1 = max(y1, y2)
    InterX2 = min(x1+w1, x2+w2)
    InterY2 = min(y1+h1, y2+h2)
    
    if InterX1 >= InterX2 or InterY1 >= InterY2:
        return score
    InterArea = (InterX2-InterX1)*(InterY2-InterY1)
    
    UionX1 = min(x1, x2)
    UionY1 = min(y1, y2)
    UionX2 = max(x1+w1, x2+w2)
    UionY2 = max(y1+h1, y2+h2)
    
    if UionX1 >= UionX2 or UionY1 >= UionY2:
        return score
    UionArea = (UionX2-UionX1)*(UionY2-UionY1)
    
    score = InterArea/UionArea
    ### END YOUR CODE
    
    print(x1, y1, w1, h1, x2, y2, w2, h2, score)
